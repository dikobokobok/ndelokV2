...
func safeJoin(explorerRoot, projectRoot, reqPath string) (string, error) {
	...
	if strings.HasPrefix(cleaned, "/") {
		...
		// Verify it is inside explorerRoot or projectRoot
		relExp, err1 := filepath.Rel(explorerRoot, cleaned)
		relProj, err2 := filepath.Rel(projectRoot, cleaned)
		if (err1 == nil && !strings.HasPrefix(relExp, "..")) || (err2 == nil && !strings.HasPrefix(relProj, "..")) {
			return cleaned, nil
		}
	...
	relExp, err1 := filepath.Rel(explorerRoot, targetExp)
	if err1 == nil && !strings.HasPrefix(relExp, "..") {
		return targetExp, nil
	}

	// Try in project root next
	targetProj := filepath.Join(projectRoot, cleaned)
	relProj, err2 := filepath.Rel(projectRoot, targetProj)
